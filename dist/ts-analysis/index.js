...
// Local imports
import { getCommonTs, initFncDets, FunctionDets, chatTask, getModelByIdx, getOaiClient, getProviderConfig
// mkMsgArray, 
 } from '../init.js';
// Exports
export function fncNameMsg(fncName) {
 ...
 */
export async function populateBodies({ provider, model, } = {}) {
    provider = provider || 'lms';
    let config = getProviderConfig(provider);
    model = model || config.model || config.defaultModel || getModelByIdx();
    let ds = await initFncDets(provider, model);
